...
from selenium_util import get_driver, navigate_to_url, perform_search, determine_page_type, extract_data_from_product_page, extract_links_from_results
...
class TestRunQuery(unittest.TestCase):
    ...
    def test_run_query_returns_true_scope_two(self):
        result = run_query(
            "Does the D678HY part work with most PartSelect dishwashers", QueryType.IN_SCOPE)
        result_content = result.content
        self.assertEqual(result_content, 'TRUE')
    ...
    def test_run_query_return_true_searchable(self):
        result = run_query(
            "Is the P67YH65 part compatible with the R874 refrigerator?", QueryType.IN_SCOPE)
        result_content = result.content
        self.assertEqual(result_content, 'TRUE')
    ...
    def test_run_query_return_true_searchable_identify_models(self):
        result = run_query(
            "What are some large fridges from PartSelect that I can buy?", QueryType.IS_SEARCHABLE)
        result_content = result.content
        self.assertEqual(result_content, 'TRUE')

    def test_run_query_return_true_searchable_identify_models_two(self):
        result = run_query(
            "What is the best fridge and the best dishwasher from part select?", QueryType.IS_SEARCHABLE)
        result_content = result.content
        self.assertEqual(result_content, 'TRUE')
    ...
    def test_extract_links(self):
        ...
        EXPECTED = ['https://www.partselect.com/PS12364199-Frigidaire-242126602-Refrigerator-Door-Shelf-Bin.htm?SourceCode=18',
